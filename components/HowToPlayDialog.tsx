...
const HowToPlayDialog: React.FC<HowToPlayDialogProps> = ({ open, onOpenChange, children }) => (
          ...
          <div className="space-y-6 pt-2">
            <div>
              <p className="text-muted-foreground font-mono leading-relaxed">
                <span className="font-bold text-foreground">Objective:</span> Identify the country that each flag belongs to by
                selecting the correct answer from the multiple choice options.
              </p>
            </div>
            <div>
              ...
              <ul className="space-y-3">
                <li>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-mono text-muted-foreground border border-foreground/20 rounded-sm px-1 py-0.5 ml-1">
                      {getDifficultySettings("easy").count} FLAGS
                    </span>
                  </div>
                  <span className="block text-xs text-muted-foreground ml-5 font-mono">
                ...
                <li>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-mono text-muted-foreground border border-foreground/20 rounded-sm px-1 py-0.5 ml-1">
                      {getDifficultySettings("medium").count} FLAGS
                    </span>
                  </div>
                  <span className="block text-xs text-muted-foreground ml-5 font-mono">
                    Like easy difficulty, but a bit more quantity, includes moderately recognizable flags.
                  </span>
                </li>
                <li>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-mono text-muted-foreground border border-foreground/20 rounded-sm px-1 py-0.5 ml-1">
                      {getDifficultySettings("hard").count} FLAGS
                    </span>
                  </div>
                  <span className="block text-xs text-muted-foreground ml-5 font-mono">
                ...
                <li>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-mono text-muted-foreground border border-foreground/20 rounded-sm px-1 py-0.5 ml-1">
                      {getDifficultySettings("expert").count} FLAGS
                    </span>
                  </div>
                  <span className="block text-xs text-muted-foreground ml-5 font-mono">
