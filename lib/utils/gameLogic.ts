...
export const getSimilarFlags = (countryCode: string): string[] => {
  const similarGroups: { [name: string]: string[] } = {
    ...
    MK: ["RU", "NL", "LU", "SI", "SK", "CZ", "HR", "RS", "BA", "ME"],
    EE: ["LV", "LT"],
    LV: ["EE", "LT"],
    LT: ["EE", "LV"],
    ...
    PG: ["AU", "NZ", "FJ", "SB", "VU", "WS", "TO", "PW", "FM", "MH", "KI", "NR", "TV"],
    SB: ["AU", "NZ", "FJ", "PG", "VU", "WS", "TO", "PW", "FM", "MH", "KI", "NR", "TV"],
    VU: ["AU", "NZ", "FJ", "PG", "SB", "WS", "TO", "PW", "FM", "MH", "KI", "NR", "TV"],
    WS: ["AU", "NZ", "FJ", "PG", "SB", "VU", "TO", "PW", "FM", "MH", "KI", "NR", "TV"],
    TO: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "PW", "FM", "MH", "KI", "NR", "TV"],
    PW: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "FM", "MH", "KI", "NR", "TV"],
    FM: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "PW", "MH", "KI", "NR", "TV"],
    MH: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "PW", "FM", "KI", "NR", "TV"],
    KI: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "PW", "FM", "MH", "NR", "TV"],
    NR: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "PW", "FM", "MH", "KI", "TV"],
    TV: ["AU", "NZ", "FJ", "PG", "SB", "VU", "WS", "TO", "PW", "FM", "MH", "KI", "NR"],
  };
...
export const calculateSimilarityScore = (
  ...
  if (difficulty === EXPERT_DIFFICULTY) {
    ...
    const lengthDiff = Math.abs(correctCountry.name.length - candidateCountry.name.length);
    if (lengthDiff <= 2) {
      score += 20;
    }
    
    // Same ending pattern bonus
    if (correctCountry.name.endsWith(candidateCountry.name.slice(-3)) || 
        candidateCountry.name.endsWith(correctCountry.name.slice(-3))) {
      score += 30;
    }
    
    // Similar syllable count bonus
    const correctSyllables = correctCountry.name.replace(/[^aeiou]/gi, '').length;
    const candidateSyllables = candidateCountry.name.replace(/[^aeiou]/gi, '').length;
    ...
    const sharedWords = correctWords.filter(word => candidateWords.includes(word));
    ...
    const colorPatterns = {
      ...
      redYellowGreen: ["GH", "CM", "GN", "ML", "SN", "BF", "NE", "TD", "TG", "BJ"],
    ...
    for (const [pattern, codes] of Object.entries(colorPatterns)) {
      if (codes.includes(correctCountry.code) && codes.includes(candidateCountry.code)) {
        score += 40;
        break;
      }
    }
    ...
    const flagElements = {
      stars: ["US", "AU", "NZ", "BR", "CN", "VN", "CU", "DO", "TT", "BB", "LC", "GD", "VC", "AG", "DM", "KN", "BS"],
      crosses: ["SE", "NO", "DK", "FI", "IS", "GB", "CH", "GR"],
      stripes: ["US", "FR", "IT", "NL", "RU", "CZ", "SK", "SI", "LU", "TH", "MY", "ID", "PH", "SG", "BR", "AR", "CL", "CO", "PE", "VE", "UY", "PY", "BO", "EC", "GY", "SR", "CR", "PA", "NI", "HN", "SV", "GT", "BZ", "JM", "CU", "DO", "TT", "BB", "LC", "GD", "VC", "AG", "DM", "KN", "BS", "HT"],
      circles: ["JP", "BD", "NP", "BN", "PW", "MH", "KI", "NR", "TV"],
      triangles: ["BA", "CZ", "SK", "SI", "LU", "TH", "MY", "ID", "PH", "SG", "BR", "AR", "CL", "CO", "PE", "VE", "UY", "PY", "BO", "EC", "GY", "SR", "CR", "PA", "NI", "HN", "SV", "GT", "BZ", "JM", "CU", "DO", "TT", "BB", "LC", "GD", "VC", "AG", "DM", "KN", "BS", "HT"],
    };
...
export const generateQuestion = (
  ...
  // Expert mode: Weighted selection towards more challenging countries
  if (difficulty === EXPERT_DIFFICULTY) {
    ...
    if (Math.random() < 0.7) {
      ...
      if (challengingCountries.length > 0) {
        correctCountry = challengingCountries[Math.floor(Math.random() * challengingCountries.length)];
      } else {
        correctCountry = remainingCountries[Math.floor(Math.random() * remainingCountries.length)];
      }
    } else {
      correctCountry = remainingCountries[Math.floor(Math.random() * remainingCountries.length)];
    }
  ...
  const finalCandidates = difficulty === EXPERT_DIFFICULTY 
    ? (viableCandidates.length >= 3 ? viableCandidates : candidatesWithScores.filter(c => c.score >= 40))
  ...
  if (difficulty === EXPERT_DIFFICULTY && finalCandidates.length > 0) {
    // Ensure at least one very high similarity option (score >= 120)
    const veryHighSimilarity = finalCandidates.filter(c => c.score >= 120);
    if (veryHighSimilarity.length > 0) {
      const selected = veryHighSimilarity[Math.floor(Math.random() * veryHighSimilarity.length)];
      incorrectOptions.push(selected.country);
    }

    // Ensure at least one high similarity option (score >= 80)
    const highSimilarity = finalCandidates.filter(c => 
      c.score >= 80 && c.score < 120 && 
      !incorrectOptions.find(opt => opt.code === c.country.code)
    );
    if (highSimilarity.length > 0) {
      const selected = highSimilarity[Math.floor(Math.random() * highSimilarity.length)];
      incorrectOptions.push(selected.country);
    }
    
    // Ensure at least one moderately high similarity option (score >= 60)
    const moderateSimilarity = finalCandidates.filter(c => 
      c.score >= 60 && c.score < 80 && 
      !incorrectOptions.find(opt => opt.code === c.country.code)
    );
    if (moderateSimilarity.length > 0) {
      const selected = moderateSimilarity[Math.floor(Math.random() * moderateSimilarity.length)];
      incorrectOptions.push(selected.country);
    }
    
    // Expert mode: Heavily penalize obvious wrong answers
    const obviousWrongAnswers = finalCandidates.filter(c => 
      c.score < 30 || (c.score < 50 && isDistinctiveFlag(c.country.code))
    );
    if (obviousWrongAnswers.length > 0) {
      // Reduce their weight significantly
      obviousWrongAnswers.forEach(c => {
        c.score = Math.max(1, c.score * 0.1); // Much more aggressive penalty
      });
    }
  } else {
    // Original logic for other difficulties
    // Ensure at least one very similar option (high similarity)
    const highSimilarityCandidates = finalCandidates.filter(c => c.score >= 60);
    if (highSimilarityCandidates.length > 0) {
      const highSimilarityCountry = highSimilarityCandidates[
    ...
    if (incorrectOptions.length < 3) {
      const mediumSimilarityCandidates = finalCandidates.filter(c => 
        c.score >= 30 && c.score < 60 && 
        !incorrectOptions.find(opt => opt.code === c.country.code)
      );
      if (mediumSimilarityCandidates.length > 0) {
        const mediumSimilarityCountry = mediumSimilarityCandidates[
...
export const calculateScore = (
  ...
  if (
    ...
    score += Math.floor(timeBonus / (difficulty === EXPERT_DIFFICULTY ? 8 : 10));
  }
...
// Enhanced country pools for expert mode
...
const getSubRegionalSimilarity = (correctCode: string, candidateCode: string): number => {
  const subRegions = {
    scandinavia: { codes: ["SE", "NO", "DK"], bonus: 60 },
    balticStates: { codes: ["EE", "LV", "LT"], bonus: 70 },
    benelux: { codes: ["BE", "NL", "LU"], bonus: 65 },
    balkans: { codes: ["HR", "SI", "RS", "BA", "ME", "MK", "AL", "BG", "RO"], bonus: 50 },
    ...
    caribbeanSmall: { codes: ["LC", "VC", "GD", "DM", "KN", "AG", "BB"], bonus: 65 },
    centralAmerica: { codes: ["GT", "BZ", "SV", "HN", "NI", "CR", "PA"], bonus: 40 },
  };
  
  for (const region of Object.values(subRegions)) {
    if (region.codes.includes(correctCode) && region.codes.includes(candidateCode)) {
      return region.bonus;
    }
  }
  return 0;
...
const getPoolSimilarityBonus = (
  ...
  Object.entries(pools).forEach(([poolName, countries]) => {
    if (countries.includes(correctCode) && countries.includes(candidateCode)) {
      switch (poolName) {
        case 'confusingFlags': bonus += 120; break;
        case 'confusingNames': bonus += 110; break;
        case 'lesserKnown': bonus += 40; break;
        case 'panAfrican': bonus += 60; break;
        case 'panArab': bonus += 70; break;
        case 'nordic': bonus += 80; break;
        case 'caribbean': bonus += 65; break;
        default: bonus += 60; break;
      }
    }
  });
...
const getHistoricalConfusionBonus = (correctCode: string, candidateCode: string): number => {
...
export const calculateExpertSimilarityScore = (
  ...
  if (correctSuffix && candidateCountry.name.toLowerCase().endsWith(correctSuffix)) {
  ...
  const subRegionalBonus = getSubRegionalSimilarity(correctCountry.code, candidateCountry.code);
  score += subRegionalBonus;
  
  // Special country pool bonuses
  score += getPoolSimilarityBonus(correctCountry.code, candidateCountry.code, pools);
  ...
  score += getHistoricalConfusionBonus(correctCountry.code, candidateCountry.code);
...
// Enhanced scoring for expert mode with progressive difficulty
export const calculateExpertScore = (
  ...
  if (difficultyPattern.length > 0) {
    const avgDifficulty = difficultyPattern.reduce((a, b) => a + b, 0) / difficultyPattern.length;
  ...
  if (difficultyPattern.length >= 5) {
    ...
    if (variance < 2) { // Low variance = consistent performance
      score += 30;
    }
  }
